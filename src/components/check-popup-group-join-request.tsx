...
type CallbackType = (groupId: number, requestorId: number) => Promise<void>;

export default function CheckPopupGroupJoinRequest({
  data,
  checkAction,
  xAction,
  groupId,
}: {
  data: GroupJoinRequestMsg[];
  ...
  const [items, setItems] = useState<
    (GroupJoinRequestMsg & { fading: boolean })[]
  >(data.map((item) => ({ ...item, fading: false })));

  // 각 아이템을 고유하게 식별할 키
  const keyOf = (item: GroupJoinRequestMsg) => `}:${item.userId}`;

  const handleSubmit = (item: GroupJoinRequestMsg, callback: CallbackType) => {
    setItems((prev) =>
      prev.map((it) =>
          ...
          >
            ...
            <div className="flex flex-col justify-center">
              <div className="mb-0.5">
                <span className="text-[#FF5679]">
                  {`${item.summonerName}#${item.summonerTag}`}
                </span>
                <span className="text-white">님의 그룹 가입 요청</span>
              </div>
